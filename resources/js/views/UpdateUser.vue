...
export default {
  ...
  methods: {
    setCustomer() {
      axios
        .put("/api/client/" + this.setUser.id, this.setUser)
        .then(() => {
    ...
    handleFileUpload() {
      ...
      this.setUser.upload = this.file.name;
  ...
  mounted() {
    axios("/api/client/" + this.$route.params.id)
      .then((response) => {
        ...
        this.setUser.tanggal = this.setTanggal(this.setUser.tanggal);
